...
export default class CreateNewPage extends React.Component {
  ...
  async handleConfirm(){
    ...
    try {
      ...
      if (result) {
        this.setState({ alertType: 'success', alertMessage: "Payroll confirmed." });
      } else {
        this.setState({ alertType: 'error',  alertMessage: "Payroll failed. Please try again with more gas." });
      }
    ...
    return (
      <article>
        <Helmet>
          ...
          <meta
            name="Create"
            content="Create a Payroll to take place on a given block on the MyBit Payroll dApp"
          />
        </Helmet>
        {toRender}
      </article>
